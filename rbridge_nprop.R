...
kintegral=function(u) 
{
  Ktilde = (3/16*u^5-5/8*u^3+15/16*u+1/2)*(abs(u)<=1)+(u>1)
}

kernelcdf = function(h,z,zfixed)
{
  n = nrow(z)
  m = ncol(zfixed)
...
rbridge=function(x,m) ##同时生成m个Brownian Bridge
{
r=length(x)
x=as.matrix(x,r,1)
var=c(x[1],diff(x))
...
N=sum(Ns);N   #total number
 
S=4    ##there are four strata

n=150  ## total sample size
r=800  ## (r+1) is the number of grid points
m=1000 ## simulatioin replications
L=20000 ## copy numbers of Brownian bridge to obtain quantiles
...
Ws=as.matrix(Ns/N)
lamda=(1/n-1/N)^(-1/2)
SRSqvalue=c(1.63,1.36,1.22,1.07)
...
#SRSh1= (SRSq[4,]-SRSq[2,])*((1/n-1/N)^(-1/2))^(-2)
...
Fn[,j]=Ws[1]*Fn1[,j]+Ws[2]*Fn2[,j]+Ws[3]*Fn3[,j]+Ws[4]*Fn4[,j]  ##ecdf
...
B3=rbridge(Fn3[,j],L)
B4=rbridge(Fn4[,j],L)
Bstar1=weight[1]*B1+weight[2]*B2+weight[3]*B3+weight[4]*B4
Lstar=apply(abs(Bstar1),2,max)
Lstar=sort(Lstar)
...
SRSFhat[,j]=kernelcdf(SRSh1,data.matrix(strsample),t(as.matrix(grid[,j])))
SRSFn[,j]=ecdf(strsample)(grid[,j])
...
for (i in 1:(r+1))
{
up1[i] = min(1, Fn[i,j]+qvalue1[2]/lamda)
lw1[i] = max(0, Fn[i,j]-qvalue1[2]/lamda)
}
...
for (i in 1:(r+1))
{
up2[i] = min(1, Fhat[i,j]+qvalue1[2]/lamda)
lw2[i] = max(0, Fhat[i,j]-qvalue1[2]/lamda)
}

SRSup = matrix(1,(r+1),1)
SRSlw = matrix(1,(r+1),1)
for (i in 1:(r+1))
{
SRSup[i] = min(1, SRSFhat[i,j]+SRSqvalue[2]/lamda)
SRSlw[i] = max(0, SRSFhat[i,j]-SRSqvalue[2]/lamda)
}
